...
Takes penguins df, drops all the na rows, then renames body_mass_g,
bill_length_mm, bill_depth_mm, and flipper_length_mm to mass, bill_l,
bill_d, and flip_l, respectively.
...
How much training data (10 folds) split data into test and training
random sampling what metric? Root mean squared error Let's make a
function for RMSE

for loop: Apply the model to each training set make prediction on the
test set with fitted training model close loop:

summarize our RMSE Final model built on whole dataset
...
#Without replace = TRUE, you can only draw 5 times. replace = TRUE means that it replaces after each draw
...
for( i in 1:folds){
  ...
  kfolds_train_df <- penguins_fold %>%
  filter(group!= i)
  
  # Run for one model
  kfolds_training_lm1 <- lm(f1, data=kfolds_train_df)

  #Get the predictions from the model
  predict_test<-kfolds_test_df |> 
  mutate(model1 = predict(kfolds_training_lm1, kfolds_test_df))
...
MODEL 2 is the best because it is the smallest
